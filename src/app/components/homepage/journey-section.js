...
const getData = async () => {
    const data = await fetch(`https://jamiat.vercel.app/api/blogs/journey`).then(
...
export default function Journey() {
    ...
    return (
        <div id="journey-section" className="w-full md:h-[680px] relative">
            ...
            >
                {journey.map((slide)=>(<SwiperSlide key={slide._id} className="swiper-slide pl-1 mobile:pl-[9vw] pr-1 mobile:pr-[4vw] min-h-[450px] h-full !flex justify-around items-center py-20 bg-journey-bg bg-cover">
                    <div className="h-fit w-[536px] md:max-w-[50%]">
                        ...
                        <div className="w-full h-[450px] border-sky-500 border-2 rounded-2xl  bg-white bg-opacity-10 shadow">
                            <div className="rounded-white-scrollbar h-[430px] p-[3vw] text-white font-light text-sm overflow-y-scroll">
